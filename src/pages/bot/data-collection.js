...
export default class DataCollection {
  constructor(workspace) {
    ...
    if (isProduction()) {
      observer.register('bot.contract', (contract) =>
        this.trackTransaction(contract)
      );
  ...
  IS_PROCESSED = true;
  endpoint =
    'https://dbot-conf-dot-business-intelligence-240201.df.r.appspot.com/dbotconf';
  loginid;
  runId = '';
  runStart = 0;
  shouldPostXml = true;
  ...
  async trackRun() {
    const xmlDom = this.cleanXmlDom(
      Blockly.Xml.workspaceToDom(this.workspace, /* opt_noId */ true)
    );
  ...
  async trackTransaction(contract) {
    ...
    const isKnownTransaction = Object.keys(this.transactionIds).includes(
  ...
  cleanXmlDom = (xmlDom) => {
    ...
    const removeAttributesRecursively = (element) => {
      uselessAttributes.forEach((uselessAttribute) =>
        element.removeAttribute(uselessAttribute)
      );
